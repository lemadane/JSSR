...
- **Space-Separated URL Attributes (`ping`)**: Strictly require `SafeUrlList` typed fields (plain `String` and `SafeUrl` are rejected). `SafeUrlList` parses space-separated URL lists and individually sanitizes every URL candidate against non-HTTP/HTTPS schemes (`mailto:`, `tel:`, `javascript:`, `data:`, etc. are converted to `about:blank`).
